...
## 1. 四个模型
| 模型 | 核心作用介绍 | SFT 模型额外改造 | 训练目标 & 损失函数 |
| ---- | ------------ | ---------------- | -------------------- |
| Actor | SFT训练后的模型，待优化的目标生成模型，负责输出对话序列 | 无 | 结合优势函数更新生成策略，约束与参考模型KL散度，无独立单模型损失，依赖PPO-Clip损失联合优化 |
| Critic | SFT训练后的模型，预测从当前token位置到序列结束的未来累计总收益期望（状态Value），用于计算优势函数 | 在每一个Token的隐层状态后拼接Value预测头 | 拟合真实时序回报与GAE估值，使用MSE损失缩小预测Value与实际回报偏差 |
| Reference | 冻结的SFT训练模型，固定输出分布，用于计算KL惩罚项，约束Actor更新幅度 | 无 | 无 |
| Reward Model | 偏好打分模型，预测单条完整问答序列的全局偏好分数 \(r(x,y)\)，提供原始奖励信号 | 在序列最后一个Token的隐层状态上拼接Reward打分头 | 学习人类偏好排序，拉大优劣回复分数差距， \(L = -log(r(x, y_{winner}) - r(x, y_{loser}))\) |
